...
public class WishlistService {
    ...
    private static final String NOMBRE_LISTA_DEFAULT = "Lista de deseos";
    ...
    /**
     * Obtiene la lista de deseos predeterminada del usuario.
     * Si no tiene ninguna, la crea automáticamente con el nombre "Lista de deseos".
     */
    public WishlistResponse obtenerMiWishlist(UUID userId) {
        Wishlist lista = obtenerOCrearListaDefault(userId);
        List<WishlistItem> items = wishlistItemRepository.findByWishlistId(lista.getId());
        return toResponse(lista, items);
    }

    /**
     * Retorna los IDs de producto guardados en la lista predeterminada del usuario.
     * Usado por el frontend para mostrar el estado del corazón en las tarjetas.
     * Si el usuario aún no tiene lista, retorna vacío sin crearla.
     */
    @Transactional(readOnly = true)
    public List<UUID> obtenerMisProductIds(UUID userId) {
        return wishlistRepository.findFirstByUserIdOrderByCreatedAtAsc(userId)
                .map(lista -> wishlistItemRepository.findByWishlistId(lista.getId())
     ...
     */
    public WishlistResponse toggleItem(UUID productId, UUID userId) {
        Wishlist lista = obtenerOCrearListaDefault(userId);

        wishlistItemRepository
                .findByWishlistIdAndProductId(lista.getId(), productId)
                .ifPresentOrElse(
                        item -> wishlistItemRepository.delete(item),
                        () -> {
                            Product product = productRepository.findById(productId)
                                    .orElseThrow(() -> new ResourceNotFoundException("Producto", productId.toString()));
                            BigDecimal precio = calcularPrecioFinal(product.getBasePrice(), product.getIvaPercent());
                            wishlistItemRepository.save(WishlistItem.builder()
                                    .wishlistId(lista.getId())
                                    .productId(productId)
                                    .priceAtAdding(precio)
                                    .build());
                        }
                );
    ...
    /** Obtiene o crea la lista predeterminada del usuario. */
    private Wishlist obtenerOCrearListaDefault(UUID userId) {
        return wishlistRepository.findFirstByUserIdOrderByCreatedAtAsc(userId)
                .orElseGet(() -> wishlistRepository.save(
                        Wishlist.builder()
                                .userId(userId)
                                .name(NOMBRE_LISTA_DEFAULT)
                                .isPublic(false)
                                .build()
                ));
    }
    ...
    private WishlistItemResponse toItemResponse(WishlistItem item) {
        ...
        boolean onSale = item.getPriceAtAdding() != null
                && precioFinal.compareTo(item.getPriceAtAdding()) < 0;

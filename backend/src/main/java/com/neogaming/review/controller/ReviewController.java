...
public class ReviewController {
    ...
    /**
     * Crea una reseña editorial directamente desde el panel de administración.
     * No requiere orden de compra. Se publica inmediatamente como APPROVED.
     */
    @PostMapping("/admin/reviews")
    @PreAuthorize("hasRole('ADMIN')")
    @SecurityRequirement(name = "bearerAuth")
    @Operation(
            summary = "Crear reseña editorial (Admin)",
            description = "Crea una reseña para cualquier producto sin verificación de compra. " +
                          "La reseña se aprueba automáticamente y es visible de inmediato."
    )
    public ResponseEntity<ApiResponse<ReviewResponse>> crearResenaAdmin(
            @Valid @RequestBody AdminCreateReviewRequest request) {
        ReviewResponse response = reviewService.crearResenaAdmin(request, SecurityUtils.getCurrentUserId());
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.created(response));
    }

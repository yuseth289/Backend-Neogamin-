...
public class CheckoutService {
    ...
    public CheckoutResponse iniciar(InitCheckoutRequest request, UUID userId) {
        // 1. Si hay un checkout activo previo sin pago asociado, cancelarlo automáticamente
        checkoutRepository.findByUserIdAndStatus(userId, EstadoCheckout.IN_PROGRESS)
                .ifPresent(prev -> {
                    var pagoOpt = paymentRepository.findByCheckoutId(prev.getId());
                    boolean pagoConfirmado = pagoOpt
                            .map(p -> p.getStatus() == com.neogaming.common.enums.EstadoPago.APPROVED
                                   || p.getStatus() == com.neogaming.common.enums.EstadoPago.PROCESSING)
                            .orElse(false);
                    if (!pagoConfirmado) {
                        // Sin pago o pago aún PENDING/CANCELLED: cancelar y permitir nuevo checkout
                        pagoOpt.ifPresent(p -> {
                            p.setStatus(com.neogaming.common.enums.EstadoPago.CANCELLED);
                            paymentRepository.save(p);
                        });
                        prev.setStatus(EstadoCheckout.CANCELLED);
                        checkoutRepository.save(prev);
                    } else {
                        throw new BusinessRuleException(
                                "Ya tienes un pago en proceso. Complétalo antes de iniciar uno nuevo.",
                                "CHECKOUT_YA_ACTIVO"
                        );
                    }
                });
        ...
        for (CartItem cartItem : cartItems) {
            ...
            UUID sellerId = sellerRepository.findById(product.getSellerId())
                    .map(s -> s.getId())
                    .orElseThrow(() -> new BusinessRuleException(
